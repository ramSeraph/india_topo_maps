...
# Load GN (Great Nicobar) grid
with open('data/layers/AN_Great_Nicobar_Grid.geojson', 'r') as f:
    gn_grid = json.load(f)
...
for feature in gn_grid['features']:
    ...
    # Generate key: GN-<SOI_SHEET_ID>.pdf
    key = f"GN-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"GN_{page_no}.pdf"
    pdf_path = f"data/GN_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/GN.json', 'w') as f:
