...
# Load LA (Little Andaman) grid
with open('data/layers/AN_Little_Andaman_Grid.geojson', 'r') as f:
    la_grid = json.load(f)
...
for feature in la_grid['features']:
    osm_25k = feature['properties']['OSM_25K_IN']
    page_no = feature['properties']['Page_no']
    
    # Extract 50K part (remove quadrant suffix)
    # Format: "C 46 I 10/SW" -> "C 46 I 10"
    if '/' in osm_25k:
    ...
    # Generate key: LA-<SOI_SHEET_ID>.pdf
    key = f"LA-{soi_sheet}.pdf"
    
    # Check if PDF exists (note: script downloads JPG but we check for jpg)
    pdf_filename = f"LA_{page_no}.jpg"
    pdf_path = f"data/LA_pdfs/{pdf_filename}"
    
    if os.path.exists(pdf_path):
        sheet_map[key] = {
            "osm_sheet_id": osm_25k,
            "soi_sheet_id": soi_sheet,
            "state_series_no": page_no,
...
with open('data/sheetmaps/LA.json', 'w') as f:
