...
# Load KA grid
with open('data/layers/KA_OSM_25K_Grid_1.geojson', 'r') as f:
    ka_grid = json.load(f)
...
for feature in ka_grid['features']:
    osm_25k = feature['properties']['OSM_25K_IN']
    index_no = feature['properties']['INDEX_NO']
    
    # Remove quotes from index_no if present
    index_no = str(index_no).replace('"', '').strip()
    
    # Format index number with leading zero
    map_no_formatted = f"{int(index_no):02d}"
    
    # Extract 50K part (remove quadrant suffix)
    # Format: "D 43 O 14 / NW" -> "D 43 O 14"
    if '/' in osm_25k:
    ...
    # Generate key: KA-<SOI_SHEET_ID>.pdf
    key = f"KA-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"{map_no_formatted}.pdf"
    pdf_path = f"data/KA_pdfs/{pdf_filename}"
    
    if os.path.exists(pdf_path):
        sheet_map[key] = {
            "osm_sheet_id": osm_25k,
            "soi_sheet_id": soi_sheet,
            "state_series_no": int(index_no),
...
with open('data/sheetmaps/KA.json', 'w') as f:
