...
# Load GA grid
with open('data/layers/GA_OSM_25K_Grid.geojson', 'r') as f:
    ga_grid = json.load(f)

# Generate sheet map
sheet_map = {}
base_url = "https://czmp.ncscm.res.in/files/GA/pdf"

for feature in ga_grid['features']:
    osm_25k = feature['properties']['OSM_25K_IN']
    index_no = feature['properties']['INDEX_NO']
    
    # Format index number with leading zero
    map_no_formatted = f"{index_no:02d}"
    
    # Extract 50K part (remove quadrant suffix)
    # Format: "D 43 C 4/SW" -> "D 43 C 4"
    if '/' in osm_25k:
    ...
    # Generate key: GA-<SOI_SHEET_ID>.pdf
    key = f"GA-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"{map_no_formatted}.pdf"
    pdf_path = f"data/GA_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/GA.json', 'w') as f:
