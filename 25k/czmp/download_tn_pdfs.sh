...
# Download TN PDFs based on grid geojson
GEOJSON_FILE="data/layers/TN_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/TN_pdfs"
BASE_URL="https://czmp.ncscm.res.in/files/TN/pdf"

# Create output directory if it doesn't exist
mkdir -p "$OUTPUT_DIR"

# Extract Map_Nos values and download PDFs
echo "Extracting Map_Nos values from $GEOJSON_FILE..."
map_numbers=$(jq -r '.features[] | .properties.Map_Nos' "$GEOJSON_FILE" | sort -n | uniq)
