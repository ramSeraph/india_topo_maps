...
# Load AP grid
with open('data/layers/AP_OSM_25K_Grid.geojson', 'r') as f:
    ap_grid = json.load(f)
...
for feature in ap_grid['features']:
    ...
    # Generate key: AP-<SOI_SHEET_ID>.pdf
    key = f"AP-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"AP_{map_no}.pdf"
    pdf_path = f"data/AP_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/AP.json', 'w') as f:
