...
# Download OD PDFs based on grid geojson
GEOJSON_FILE="data/layers/OD_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/OD_pdfs"
BASE_URL="https://czmp.ncscm.res.in/files/OD/pdf"

# Create output directory if it doesn't exist
mkdir -p "$OUTPUT_DIR"

# Extract Map values and download PDFs
echo "Extracting Map values from $GEOJSON_FILE..."
map_numbers=$(jq -r '.features[] | .properties.Map' "$GEOJSON_FILE" | sort -n | uniq)
