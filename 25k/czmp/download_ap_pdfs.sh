...
# Download AP PDFs based on grid geojson
GEOJSON_FILE="data/layers/AP_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/AP_pdfs"
BASE_URL="https://czmp.ncscm.res.in/files/AP/pdf"
...
for map_no in $map_numbers; do
    count=$((count + 1))
    url="${BASE_URL}/AP_${map_no}.pdf"
    output_file="${OUTPUT_DIR}/AP_${map_no}.pdf"
    
    # Skip if file already exists
    if [ -f "$output_file" ]; then
        echo "[$count/$total] Skipping AP_${map_no}.pdf (already exists)"
        continue
    fi
    
    echo "[$count/$total] Downloading AP_${map_no}.pdf..."
