...
# Load MH grid
with open('data/layers/MH_OSM_25K_Grid.geojson', 'r') as f:
    mh_grid = json.load(f)
...
for feature in mh_grid['features']:
    ...
    # Generate key: MH-<SOI_SHEET_ID>.pdf
    key = f"MH-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"MH_{map_no}.pdf"
    pdf_path = f"data/MH_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/MH.json', 'w') as f:
