...
# Download GJ PDFs based on grid geojson
GEOJSON_FILE="data/layers/GJ_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/GJ_pdfs"
...
map_numbers=$(jq -r '.features[] | .properties.INDEX_NO' "$GEOJSON_FILE" | sort -n | uniq)
...
for map_no in $map_numbers; do
    count=$((count + 1))
    url="${BASE_URL}/GJ_${map_no}.pdf"
    output_file="${OUTPUT_DIR}/GJ_${map_no}.pdf"
    
    # Skip if file already exists
    if [ -f "$output_file" ]; then
        echo "[$count/$total] Skipping GJ_${map_no}.pdf (already exists)"
        continue
    fi
    
    echo "[$count/$total] Downloading GJ_${map_no}.pdf..."
