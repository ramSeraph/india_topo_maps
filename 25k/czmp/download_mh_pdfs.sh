...
# Download MH PDFs based on grid geojson
GEOJSON_FILE="data/layers/MH_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/MH_pdfs"
BASE_URL="https://czmp.ncscm.res.in/files/MH/pdf"
...
for map_no in $map_numbers; do
    count=$((count + 1))
    url="${BASE_URL}/MH_${map_no}.pdf"
    output_file="${OUTPUT_DIR}/MH_${map_no}.pdf"
    
    # Skip if file already exists
    if [ -f "$output_file" ]; then
        echo "[$count/$total] Skipping MH_${map_no}.pdf (already exists)"
        continue
    fi
    
    echo "[$count/$total] Downloading MH_${map_no}.pdf..."
