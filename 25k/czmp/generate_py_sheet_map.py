...
# Load PY grid
with open('data/layers/PY_OSM_25K_Grid.geojson', 'r') as f:
    py_grid = json.load(f)
...
for feature in py_grid['features']:
    ...
    # Generate key: PY-<SOI_SHEET_ID>.pdf
    key = f"PY-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"{map_no}.pdf"
    pdf_path = f"data/PY_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/PY.json', 'w') as f:
