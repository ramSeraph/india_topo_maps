...
# Load KL grid
with open('data/layers/KL_OSM_25K_Grid.geojson', 'r') as f:
    kl_grid = json.load(f)

# Generate sheet map
sheet_map = {}
base_url = "https://czmp.ncscm.res.in/files/KL/pdf"

for feature in kl_grid['features']:
    ...
    # Generate key: KL-<SOI_SHEET_ID>.pdf
    key = f"KL-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"KL_{map_no_formatted}.pdf"
    pdf_path = f"data/KL_pdfs/{pdf_filename}"
...
with open('data/sheetmaps/KL.json', 'w') as f:
