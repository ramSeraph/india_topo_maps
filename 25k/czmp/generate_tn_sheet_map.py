...
# Load TN grid
with open('data/layers/TN_OSM_25K_Grid.geojson', 'r') as f:
    tn_grid = json.load(f)

# Generate sheet map
sheet_map = {}
base_url = "https://czmp.ncscm.res.in/files/TN/pdf"

for feature in tn_grid['features']:
    osm_25k = feature['properties']['OSM_25K_IN']
    map_nos = feature['properties']['Map_Nos']
    
    # Extract 50K part (remove quadrant suffix)
    # Format: "D 44 U 2 / NE" -> "D 44 U 2"
    if '/' in osm_25k:
    ...
    # Generate key: TN-<SOI_SHEET_ID>.pdf
    key = f"TN-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"{map_nos}.pdf"
    pdf_path = f"data/TN_pdfs/{pdf_filename}"
    
    if os.path.exists(pdf_path):
        sheet_map[key] = {
            "osm_sheet_id": osm_25k,
            "soi_sheet_id": soi_sheet,
            "state_series_no": map_nos,
...
with open('data/sheetmaps/TN.json', 'w') as f:
