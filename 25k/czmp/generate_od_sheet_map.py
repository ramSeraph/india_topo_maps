...
# Load OD grid
with open('data/layers/OD_OSM_25K_Grid.geojson', 'r') as f:
    od_grid = json.load(f)

# Generate sheet map
sheet_map = {}
base_url = "https://czmp.ncscm.res.in/files/OD/pdf"

for feature in od_grid['features']:
    osm_25k = feature['properties']['OSM_25K_IN']
    map_no = feature['properties']['Map']
    
    # Format map number with leading zero
    map_no_formatted = f"{int(map_no):02d}"
    
    # Extract 50K part (remove quadrant suffix)
    # Format: "F 45 U 8/SW" -> "F 45 U 8"
    if '/' in osm_25k:
    ...
    # Generate key: OD-<SOI_SHEET_ID>.pdf
    key = f"OD-{soi_sheet}.pdf"
    
    # Check if PDF exists
    pdf_filename = f"{map_no_formatted}.pdf"
    pdf_path = f"data/OD_pdfs/{pdf_filename}"
    
    if os.path.exists(pdf_path):
        sheet_map[key] = {
            "osm_sheet_id": osm_25k,
            "soi_sheet_id": soi_sheet,
            "state_series_no": int(map_no),
...
with open('data/sheetmaps/OD.json', 'w') as f:
