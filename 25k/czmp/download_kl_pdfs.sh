...
# Download KL PDFs based on grid geojson
GEOJSON_FILE="data/layers/KL_OSM_25K_Grid.geojson"
OUTPUT_DIR="data/KL_pdfs"
...
for map_no in $map_numbers; do
    ...
    url="${BASE_URL}/KL_${formatted_no}.pdf"
    output_file="${OUTPUT_DIR}/KL_${formatted_no}.pdf"
    
    # Skip if file already exists
    if [ -f "$output_file" ]; then
        echo "[$count/$total] Skipping KL_${formatted_no}.pdf (already exists)"
        continue
    fi
    
    echo "[$count/$total] Downloading KL_${formatted_no}.pdf..."
